import React, { useEffect, useState } from "react";
import $ from "jquery";
import "datatables.net-dt/css/dataTables.dataTables.css";
import "datatables.net-dt";
import Sidebar from "./Sidebar";
import axios from "axios";
import { Link } from '@inertiajs/react';
const Driver = () => {
    const [drivers, setDrivers] = useState([]);

    // Fetch drivers from API
    useEffect(() => {
        axios.get("/api/drivers")
            .then(response => {
                console.log("Fetched drivers:", response.data);
                setDrivers(response.data);
            })
            .catch(error => {
                console.error("Error fetching drivers:", error);
            });
    }, []);

    // Initialize DataTable after data is loaded
    useEffect(() => {
        const timeout = setTimeout(() => {
            if ($.fn.DataTable.isDataTable("#driverTable")) {
                $("#driverTable").DataTable().destroy();
            }
            $("#driverTable").DataTable();
        }, 100); // slight delay ensures DOM is ready

        return () => clearTimeout(timeout);
    }, [drivers]);

    return (
        <Sidebar>
            <div className="p-6">
                {/* Header */}
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold">All Drivers</h1>
                </div>

                {/* Button Group */}
                <div className="flex space-x-2 mb-6">
                    <button className="bg-red-600 text-white px-4 py-2 rounded">
                        <i className="fa fa-list mr-2"></i> Drivers List
                    </button>

                    <Link href="/drivers/createDriverPage">
                        <button className="bg-gray-200 text-black px-4 py-2 rounded hover:bg-gray-300">
                            <i className="fa fa-plus mr-2"></i> Create Driver
                        </button>
                    </Link>
                </div>

                {/* DataTable */}
                <table id="driverTable" className="display w-full border">
                    <thead className="bg-gray-100">
                        <tr>
                            <th>Image</th>
                            <th>Name</th>
                            <th>Email</th>
                            <th>Date</th>
                            <th>Documents</th>
                            <th>Active</th>
                            <th>Online</th>
                            <th>Wallet History</th>
                            <th>Total Orders</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {Array.isArray(drivers) && drivers.length > 0 ? (
                            drivers.map((driver, index) => (
                                <tr key={index}>
                                    <td>
                                        <img
                                            src={driver.image || "https://via.placeholder.com/48?text=No+Image"}
                                            alt="driver"
                                            className="w-12 h-12 rounded-full object-cover"
                                            onError={(e) => {
                                                e.target.onerror = null;
                                                e.target.src = "https://via.placeholder.com/48?text=No+Image";
                                            }}
                                        />

                                    </td>
                                    <td>{driver.name}</td>
                                    <td>{driver.email}</td>
                                    <td>{driver.date ? new Date(driver.date).toLocaleDateString() : "N/A"}</td>
                                    <td>{driver.documents || "N/A"}</td>
                                    <td>{driver.active ? "Yes" : "No"}</td>
                                    <td>{driver.online ? "Yes" : "No"}</td>
                                    <td>{driver.wallet_history || "₹0"}</td>
                                    <td>{driver.total_orders ?? 0}</td>
                                    <td>
                                        <button className="px-2 py-1 bg-blue-500 text-white rounded">
                                            Edit
                                        </button>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan="10" className="text-center py-4 text-gray-500">
                                    No drivers found or still loading...
                                </td>
                            </tr>
                        )}
                    </tbody>
                </table>
            </div>
        </Sidebar>
    );
};

export default Driver;
